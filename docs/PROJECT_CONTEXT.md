...
- `/`, `/privacy`, `/terms` — marketing/static. `/c.vcf` is the public vCard (`text/vcard`, inline) that Camera/NFC should open directly — do not put a landing page in front of that URL. `/card` is an unlisted print/test page (noindex, robots-disallowed, not in sitemap/nav); do not link it from public pages. `/review` is an unlisted permanent redirect to the Google leave-a-review form (QR at `/qr/review.png` encodes `https://swaffordspeed.com/review`); robots-disallowed, not in sitemap/nav.
