...
- `docs/crm-roadmap.md` Phase 3B (screenshot intake) is built: Telegram + Resend email land in Open Drafts; approve creates the customer, job, and ticket. CRM now also scores health, saves verbatim quotes, and ranks who to ask for a Google review first. Remaining phases (board clutter window, match-confirmation UI) are still proposed.
